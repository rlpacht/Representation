...
PoliticsApp.controller('GraphCtrl', ['$scope', '$timeout', '$http', 'usSpinnerService', function ($scope, $timeout, $http, usSpinnerService) {
  ...
  // Set data for quick search links
  $scope.popularSearch = function(politician, year) {
    $scope.politicianData = {
      name: {title: politician},
      electionCycle: year,
      limit: null
    };
    $scope.getData();
  ...
  // Returns true if charts should be displayed
  $scope.displayCharts = function() {
    return !$scope.isLoading && !$scope.hasError;
  };

  // Fetch data from backend, populate for charts
  $scope.getData = function() {
    ...
    $scope.startSpin();
    $http.get("/contributions.json", {
      params: politicianInfo
      ...
      topContributions.forEach(function(contribution) {
        ...
        var amount = Math.floor((contribution[1] / 100));
      ...
      $http.get("/total.json", {
        ...
        var totalMinusTopContributors = Math.floor(($scope.total / 100)) - $scope.totalFromTopContributors;
      ...
      // Displays two totals because I'm adding it twice:
      // once after subtracting PAC contributions, and again
      // when I concat $scope.topContributionAmounts because it already contains a total
      $http.get("/amount_from_pacs.json", {
        params:politicianInfo
      }).success(function (res) {
        $scope.pacContributionAmounts = [];
        $scope.pacContributionNames = [];
